...
void main(void) 
{
	...
	P2DIR = BIT0 + BIT2 + BIT6 + BIT7;
	P2OUT = BIT0 + BIT2 + BIT6 + BIT7;    // set the 4 gpio pins as outputs
    ...
    //change TACCR0 to TB0CCRO
	TB0CCR0 = 62500 - 1;    // A period of 62,500 cycles is 0 to 62,499. TACCR0 = 1500-1 is about as fast as it can be, visually.
	TB0CCTL0 = CCIE;        // Enable interrupts for CCR0.
	TB0CTL = TBSSEL_2 + ID_3 + MC_2 + TBCLR;  // SMCLK, multiply time delay by 2^3=8, continuous mode, timer reset. multiply by 8 achieved by ID_3.
